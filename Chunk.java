        ...
        for (float x = 0; x < CHUNK_SIZE; x++) {
            for (float z = 0; z < CHUNK_SIZE; z++) {
                for (float y = 0; y < CHUNK_SIZE; y++) {
                    ...
                    if (x >= waterXmin && x <= waterXmax && z >= waterZmin && z <= waterZmax && y == 6 && (height - 1) == 6) {
                        Blocks[(int) x][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Water);
                    }
                    //Generate random sand area
                    else if( x>=sandXmin && x<=sandXmax && z >= sandZmin && z <= sandZmax && y == 6){
...
//                            (float) 0, Blocks[(int) (x)][(int) (y)][(int) (z)]));
        ...
        for (float x = 0; x < CHUNK_SIZE; x++) {
            for (float z = 0; z < CHUNK_SIZE; z++) {
                ...
                Block current = Blocks[(int) x][(int) y][(int) z];

                //Generate sand around water
                if (current.GetType() == Block.BlockType.BlockType_Water) {
                    float ran = r.nextFloat();
                    Block right, left, front, behind;
                    if (x < CHUNK_SIZE - 1) {
                        right = Blocks[(int) x + 1][(int) y][(int) z];
                        if (right.GetType() == Block.BlockType.BlockType_Grass) {
                            Blocks[(int) x + 1][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Sand);
                            Blocks[(int) x][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Sand);
                            if(ran < .3 && x < CHUNK_SIZE -2){
                                Blocks[(int) x + 2][(int) y][(int)y] = new Block(Block.BlockType.BlockType_Sand);                                
                            }
                        }
                    }
                    if (x > 0) {
                        left = Blocks[(int) x - 1][(int) y][(int) z];
                        if (left.GetType() == Block.BlockType.BlockType_Grass) {
                            Blocks[(int) x - 1][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Sand);
                            Blocks[(int) x][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Sand);
//                            if(ran < .3){
//                                Blocks[(int) x][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Sand);                                
//                            }
                        }else if((left.GetType() == Block.BlockType.BlockType_Dirt) && ran > .5 && x > 2){
                            Blocks[(int) x - 2][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Sand);
                            Blocks[(int) x][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Sand);
                        }
                    }
                    if (z < CHUNK_SIZE - 1) {
                        front = Blocks[(int) x][(int) y][(int) z + 1];
                        if (front.GetType() == Block.BlockType.BlockType_Grass) {
                            Blocks[(int) x][(int) y][(int) z + 1] = new Block(Block.BlockType.BlockType_Sand);
                            Blocks[(int) x][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Sand);
                            if(ran < .3 && z < CHUNK_SIZE - 2){
                                Blocks[(int)x ][(int) y][(int) z + 2] = new Block(Block.BlockType.BlockType_Sand);                                
                            }
                        }
                    }
                    if (z > 0) {
                        behind = Blocks[(int) x][(int) y][(int) z - 1];
                        if (behind.GetType() == Block.BlockType.BlockType_Grass) {
                            Blocks[(int) x][(int) y][(int) z -1 ] = new Block(Block.BlockType.BlockType_Sand);
                            Blocks[(int) x][(int) y][(int) z ] = new Block(Block.BlockType.BlockType_Sand);
                            if(ran < .3 && z > 1){
                                Blocks[(int) x][(int) y][(int) z - 2] = new Block(Block.BlockType.BlockType_Sand);                                
...
//                if (isSand) {
//                    Blocks[(int) x][(int) y][(int) z] = new Block(Block.BlockType.BlockType_Sand);
//                    System.out.println("create sand block");
//                    System.out.println("type: " + Blocks[(int) x][(int) y][(int) z].GetType());
//                }
            }
        }
        for (float x = 0; x < CHUNK_SIZE; x++) {
        ...
        //generateSand(startX, startZ, startY);
        ...
        for (int x = 0; x < CHUNK_SIZE; x++) {
            for (int y = 0; y < CHUNK_SIZE; y++) {
                for (int z = 0; z < CHUNK_SIZE; z++) {
                    ...
                    } else if (rand > 0.5f && (y >=2 && y <= 6)) {
                        Blocks[x][y][z] = new Block(Block.BlockType.BlockType_Stone);
//                    } else if (rand <= 0.5f && (y == 2 || y == 3)) {
